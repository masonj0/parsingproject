#!/usr/bin/env python3
"""
Paddock Parser Toolkit - Integrated Collector Generator (link_helper.py)

NOTE TO SELF (The Constitution - Article III: The Collector & Proxies):
This module generates our interactive data collection dashboard, the brilliant
"Integrated Collector." It is NOT just a list of links. The dual-proxy system
is our "Firewall Buster," a key strategic feature that makes manual collection
viable from any network. The "Copy to Clipboard" workflow is the "Seamless
Bridge" that creates a frictionless user experience.
"""
...
def create_and_launch_link_helper(config: Dict):
    ...
    This page includes text boxes and a "Copy to Clipboard" button to facilitate
    the innovative clipboard-first workflow.
    """
    ...
    for category in source_categories:
        ...
        for site in sites:
            ...
            source_id = name.replace(" ", "_").replace("(", "").replace(")", "") # Create a stable ID
...
<body>
    ...
    <script>
        document.getElementById('copyButton').addEventListener('click', () => {{
            ...
            textareas.forEach(ta => {{
                if (ta.value.trim() !== '') {{
                    const sourceId = ta.id.replace(/_/g, ' '); // Recreate name from ID
                    masterString += `---START ${sourceId}---\\n`;
    ...
    try:
        ...
        logging.info(f"Generated Integrated Collector: {helper_path.resolve()}")
        webbrowser.open(f"file://{helper_path.resolve()}")
    except Exception as e:
        logging.error(f"Could not create or open the Integrated Collector file: {e}")```
